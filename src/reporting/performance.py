...
class PerformanceAnalyzer:
    ...
    def analyze_best_conditions(self, results):
        ...
        best_session = profitable_trades.groupby(profitable_trades.index.hour)['profit'].mean().idxmax()
        
        return f"Best Market Regime: {best_regime}\nBest Trading Hour: {best_session:02d}:00"
    
    def analyze_trade_distribution(self, results):
        trade_counts = results[results['trade_executed']].groupby(
            [results.index.hour, 'regime'])['profit'].count().unstack(fill_value=0)
